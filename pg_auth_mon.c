...
/*
 * Write content of the `pg_auth_mon` to regular Postgres log to make it
 * searchable later.
 */
static void
log_pg_auth_mon_data(){
 ...
 *
 * The SQL API of this function has changed in version 1.1, and may change
 * again in the future.
 *
 ...
 * That is, Postgres starts with the new pg_auth_mon.so, but
 * "ALTER EXTENSION pg_auth_mon UPDATE" wasn't executed yet.
 * It is a typical scenario we see during the rolling upgrade: a replica is
 * running with the new .so file, but the primary with the old one.
 *
...
pg_auth_mon_internal(PG_FUNCTION_ARGS, pgauthmonVersion api_version)
{
	...
	/* fail if the assert is not updated in the new version */
	Assert(i == (api_version == PG_AUTH_MON_V1_0 ? PG_AUTH_MON_COLS_V1_0 :
				api_version == PG_AUTH_MON_V1_1 ? PG_AUTH_MON_COLS_V1_1 : -1));
