...
select rolname, successful_attempts, total_hba_conflicts, other_auth_failures from pg_auth_mon(), pg_roles where oid = uid and rolname like 'auth_%';

--2.Login attempt by invalid username
\! PGPASSWORD=postgres psql -U auth_blah -d testdb  -c "select 1"
select uid, successful_attempts, total_hba_conflicts, other_auth_failures from pg_auth_mon() where uid = 0;
